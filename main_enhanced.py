...
class TradingBot:
    ...
    def _init_components(self):
        ...
        try:
            ...
            # Initialize WebSocket client - only pass the db_manager
            self.components['websocket'] = BinanceWebSocketClient(self.db_manager)

            # Initialize technical indicators - pass db_path string
            self.components['indicators'] = TechnicalIndicators(self.db_path)

            # Initialize AI model - pass db_path string
            self.components['ai_model'] = EnhancedAIModel(self.db_path)

            # Initialize signal generator - check what parameters are expected
            ...
            if self.telegram_token:
                try:
                    ...
                    if len(self.telegram_token) > 20:
                        self.components['telegram'] = TelegramAdapter(
                            token=self.telegram_token,
                            allowed_users=self.allowed_users,
                            components=self.components
                        )
                        logger.info("Telegram component initialized successfully")
                    else:
                        logger.warning("Invalid Telegram token format, Telegram notifications will be disabled")
                except Exception as e:
                    logger.error(f"Error initializing Telegram component: {e}", exc_info=True)
            ...
            try:
                ...
                for i in range(0, min(20, len(symbols)), symbols_per_batch):
                    ...
                    for symbol in batch_symbols:
                        ...
                        try:
                            self.components['indicators'].calculate_indicators(symbol, "1h")
                        except Exception as e:
...
if __name__ == "__main__":
    ...
    ALLOWED_USERS = os.environ.get("TELEGRAM_CHAT_ID", "").split(",") if os.environ.get("TELEGRAM_CHAT_ID") else []

    # Log environment variables (with partial redaction for security)
    if TELEGRAM_TOKEN:
        logger.info(f"Telegram token found: {TELEGRAM_TOKEN[:5]}...")
    else:
        logger.warning("No Telegram token found in environment variables")
