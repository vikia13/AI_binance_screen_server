...
class AITradingSystem:
    def __init__(self, db_path='data', config_path='config.json'):
        ...
        self.model_retrain_interval = 86400  # Retrain models every 24 hours
        ...
        # Initialize Telegram bot AFTER other components
    ...
    def init_telegram(self):
        ...
        if not TELEGRAM_AVAILABLE:
            logger.warning("Telegram functionality not available")
            return None
        ...
        try:
            # Use SimpleTelegramBot instead of EnhancedTelegramBot
            bot = EnhancedTelegramBot(
                token=telegram_token,
                chat_id=telegram_chat_id,
                signal_generator=self.signal_generator,
                indicator_manager=self.indicators,
                websocket_client=self.websocket_client
            )

            if bot.start():
                logger.info(f"Simple Telegram bot initialized with chat ID: {telegram_chat_id}")
                return bot
            else:
                logger.error("Failed to start Simple Telegram bot")
                return None
